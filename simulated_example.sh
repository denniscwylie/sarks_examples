...
# dumping output (-o) to a new directory named sarks_simulated_selection
java -jar sarks.jar\
     select\
     -f simulated_seqs.fa\
...
# merged_peaks.tsv
# peaks.tsv
# permdists_spatial_windowed.tsv
# permdists_windowed.tsv
# theta.tsv
## -----------------------------------------------------------------------------
# Each of these files is a tab-separated-values (tsv) table.
# For now, we consider only two:
## -----------------------------------------------------------------------------
# sarks_simulated_selection/theta.tsv
...
## -----------------------------------------------------------------------------
# The other two files (..._windowed.tsv) contain summary statistics
# for the distribution of smoothed scores in each null permutation,
# and will be used by the sarks.jar test command below.
...
## -----------------------------------------------------------------------------
# -r <number R_2 of permutations used for testing,
#     need not be same as number R of permutations used to set threshold>
# -i <input directory for testing is output directory from selection step;
#     needed to determine selection thresholds for testing>
# -e <optional random number generator seed for reproducibility;
#     *should NOT be same number as used for seed in select step!*>
## -----------------------------------------------------------------------------
# The sarks.jar test command also emits a number to stdout, such as:
#
# 16
# 
# Given -r 1000, this tells us that 16 / 1000 random permutations
# generated nonempty motif sets using the thresholds we calculated
# using sarks.jar select command, yielding an estimated false positive
# rate (FPR) of 1.6%.
#
# Recommend calculating a 95% confidence interval (CI) for the FPR
